...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False, cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        ...
        try:
            ...
            self.im_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in IMG_FORMATS)    # 15615, 4267
        ...
        # Check cache
        self.label_files = img2label_paths(self.im_files)  # labels # 15615, 4267
        ...
        labels, shapes, self.segments = zip(*cache.values())    # labels:  15615
        nl = len(np.concatenate(labels, 0))  # number of labels # nl
    ...
    #     img_tml, labels_out, im_files, shapes = self.get_modal_item(index, 'thermal')
    #     img_rad, _, _, _ = self.get_modal_item(index, 'radar')
    #     img_fusion = [img_tml, img_radar]
    #     return

    def __getitem__(self, index):
        # def get_modal_item(self, index):
        index = self.indices[index]  # linear, shuffled, or image_weights

        hyp = self.hyp
        mosaic = self.mosaic and random.random() < hyp['mosaic']    # hyp['mosaic']=1.0
        if mosaic:  # True
            # Load mosaic
            img, labels = self.load_mosaic(index)  # NOTE x2img(img, 'demo1.png')   # (640, 640, 3)
            shapes = None

            # MixUp augmentation
            if random.random() < hyp['mixup']:  # hyp['mixup']=0.0
            ...
            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape # 544, 672
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)   # NOTE Why letterbox?
