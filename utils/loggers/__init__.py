...
# LOGGERS = ('csv', 'wandb')  # *.csv, TensorBoard, Weights & Biases, ClearML
...
class Loggers():
    ...
    def on_model_save(self, last, epoch, final_epoch, best_fitness, fi):
        ...
        if (epoch + 1) % self.opt.save_period == 0 and not final_epoch and self.opt.save_period != -1:
            if self.wandb:
                ...
                pass    # I dont want to save model on wandb
            if self.clearml:
    ...
    def on_train_end(self, last, best, epoch, results):
        ...
        if self.wandb:
            ...
            if not self.opt.evolve:
                ...
                pass # I don't want to save model on wandb
    ...
    def on_record_variance(self, name, value):
        if self.wandb:
            self.wandb.log({f"{name}": wandb.Histogram(value.detach().cpu().numpy())})
